...
def get_trigger_channel(basel_dac_object: Parameter | Sweep):
    ...
    if ch in np.arange(1,13,1):  # Low dac board - contains AWG A and B - connect sync out A to mfli trigger input 3
        return 3 #1
    elif ch in np.arange(13,25,1):  # # High dac board - contains AWG C and D - connect sync out C to mfli trigger input 4
        return 4 #2
