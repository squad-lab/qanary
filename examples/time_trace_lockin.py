...
from drivers.squad.helpers.helpers import Lockin, Delay

from qcutils.sweep import Sweep, rampdown, sweeper
from qcutils.buffered.instruments import NodeDelay, NodeUHFLI, NodeMFLI
...
lockin_RF = Lockin(name='uhfli', address='localhost', device="UHFLI", serial='DEV2793', demod_channels=[0])
...
def UHFLI_time_trace_measurement(target_duration: float = 5, n_samples: int = 1000):
    ...
    time_sweep = Sweep(t, point_delay, n_samples*point_delay, n_samples, start_delay=0, delay=point_delay)
    ...
    run_dict["experiment_name"] = f"UHFLI lockin time trace measurement"
    ...
    print(f"UHFLI lockin time trace measurement completed in {time.time() - t_start:.2f} s.")
...
def MFLI_time_trace_measurement(target_duration: float = 5, n_samples: int = 1000):
    ...
    run_dict["experiment_name"] = f"MFLI lockin time trace measurement"
    ...
    print(f"MFLI lockin time trace measurement completed in {time.time() - t_start:.2f} s.")
...
def time_trace_measurement(target_duration: float = 1, n_samples: int = 100):
    ...
    time_sweep = Sweep(t, point_delay, n_samples*point_delay, n_samples, start_delay=point_delay, delay=point_delay)

    run_dict["experiment_name"] = f"Lockin stepped time trace measurement"
    ...
    print(f"Lockin stepped time trace measurement completed in {time.time() - t_start:.2f} s.")
